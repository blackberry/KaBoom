...
public class PriParser {
	...
	public boolean parsePri(byte[] bytes, int pos, int length) throws Exception{
					...
					if (digit1 == '1') {
						if (bytes[pos + 2] == '>') {
							...
							if (bytes[pos + 2] >= '0' && bytes[pos + 2] <= '9'
								...
								if (bytes[pos + 2] >= '0' && bytes[pos + 2] <= '8'
									...
									if (bytes[pos + 2] == '9' && bytes[pos + 3] >= '0'
